...
import { Shield, User, Calendar, Mail, CreditCard, Tag, List, DollarSign, Star, Eye, Zap, Crown, Moon, Pickaxe } from 'lucide-react';
...
import CraftorProfileLoader from './CraftorProfileLoader';
...
export interface CraftorData {
  ...
  tier: 'Basic' | 'Basic-Advanced' | 'Pro' | 'Elite';
}
...
const CraftorProfile: React.FC = () => {
  ...
  const getTierIcon = (tier: string) => {
    switch(tier) {
      case 'Basic-Advanced' : return <Pickaxe className='w-5 h-5' />
  ...
  const getTierGradient = (tier: string) => {
    switch(tier) {
      case 'Basic-Advanced' : return 'from-orange-400 via-red-500 to-yellow-500';
